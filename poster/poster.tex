...
Two types of replication are used in our distributed system. The executor service uses queue-based replication. The executors are stateless and rely on the priority queue for order assignment. Orders persist in the queue until successfully processed and executors scale dynamically based on queue size. The database uses a synchronous leader-follower replication. The leader handles all writes and the followers serve read requests. Followers mirror leader state before acknowledging reads and automatic elections during the leader replica failure minimize database downtime.
